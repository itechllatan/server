...
const getFrequencyRisksByHeatMap = async function (httpRequest){
  ...
  const frequency = await useCase.getFrequencyRisksByHeatMap(id);
...
const getImpactRisksByHeatMap = async function (httpRequest){
  const { id } = httpRequest.params;
  if (!id) {
    return {
      statusCode: 400,
  ...
  const impact = await useCase.getImpactRisksByHeatMap(id);
...
module.exports = {
  ...
  getHeatMap,
  getUnitTime,
  getFrequencyRisksByHeatMap,
  getImpactRisksByHeatMap
}