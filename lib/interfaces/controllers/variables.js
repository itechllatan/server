...
const getVariablesAndOptions = async function (httpRequest) { //se optiene variables y sus opciones segun el tipo
...
const getVariables = async function (httpRequest) { //se optienen las variables segun su tipo
  const { type } = httpRequest.params;
  if (!type) {
    return {
      statusCode: 400,
