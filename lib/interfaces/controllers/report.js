...
const getReport = async function (httpRequest) {
  ...
  const ucMatrixRL = new MatrixUC({ riskLevelRepository })
  const riskLevelTmp = await ucMatrixRL.getRiskLevelByHeatMap(idHeatMap);
  const riskLevel = riskLevelTmp.sort((a, b) => b.id_risk_level - a.id_risk_level);
  ...
  let filTitle = 2
  let colTitle = 2
  ws.column(colTitle).setWidth(4.5);
  ws.row(filTitle).setHeight(28);
  ws.cell(filTitle, colTitle, filTitle, (colTitle + lenImpact), true)
  ...
  let filTitleRes = 2
  let colTitleRes = (colTitle + lenImpact) + 4
  ws.column(colTitleRes).setWidth(4.5);
  ws.cell(filTitleRes, colTitleRes, filTitleRes, (colTitleRes + lenImpact), true)
  ...
  filTitle += 1;
  ws.cell(filTitle, (colTitle + 2), filTitle, (colTitle + 1 + lenImpact), true)
    .string('IMPACTO')
    .style({
  ...
  filTitle += 1;
  ws.cell((filTitle + 1), colTitle, (filTitle + lenFrequency), colTitle, true)
    .string('FRECUENCIA')
    .style({
  ...
  filTitleRes += 1;
  ws.cell(filTitleRes, (colTitleRes + 2), filTitleRes, (colTitleRes + 1 + lenImpact), true)
    .string('IMPACTO')
    .style({
      alignment: { wrapText: true, vertical: 'center', horizontal: 'center', },
      border: {
  ...
  filTitleRes += 1;
  ws.cell((filTitleRes + 1), colTitleRes, (filTitleRes + lenFrequency), colTitleRes, true)
    .string('FRECUENCIA')
    .style({
      alignment: { wrapText: true, textRotation: 90, vertical: 'center', horizontal: 'center', },
      border: {
  ...
  let totalRisk = risksHM.length;
  let nameFrequency;
  let nameImpact;
  let colImp;
  let filImp;
  let colFre = colTitle + 1;
  let filFre = filTitle + 1;
  let columna;
  let fila = filTitle + 1;
  let styleNameMap = {
    alignment: { wrapText: true, vertical: 'center', horizontal: 'center', },
  ...
  frequencyRisk?.forEach((f) => {
    ...
    impactRisk?.forEach((i) => {
      ...
      ws.cell(fila, columna).string((arrayRisk?.length === 0 ? '' : arrayRisk?.length.toString()))
        .style(styleCubo);

      ws.cell(filImp, (colImp + lenImpact + 4))
        .string(nameImpact)
        .style(styleNameMap);
      ws.column((colImp + lenImpact + 4)).setWidth(12);
      ws.row(filImp).setHeight(35);

      ws.cell(filFre, (colFre + lenImpact + 4))
        .string(nameFrequency)
        .style(styleNameMap);
      ws.column((colFre + lenImpact + 4)).setWidth(10);
      ws.row(filFre).setHeight(35);

      let arrayRiskRe = risksHM?.filter(e => e.residualRisk.frequencyRisk.id_frequency_risk === f.id_frequency_risk && e.residualRisk.impactRisk.id_impact_risk === i.id_impact_risk);
      ws.cell(fila, (columna + lenFrequency + 4)).string((arrayRiskRe?.length === 0 ? '' : arrayRiskRe?.length.toString()))
        .style(styleCubo);
  ...
  for (const rl of riskLevel) {
    let color = rgbToHex(rl.color) || '#ffffff';
    let styleName = {
      alignment: { wrapText: true, vertical: 'center', horizontal: 'center', },
      fill: { type: 'pattern', patternType: 'solid', bgColor: color, fgColor: color },
      border: {
    ...
    ws.cell(filLevel, colLevel)
      .string(rl.name)
      .style(styleName);

    colLevelRes = (colLevel + lenImpact + 4)
    ws.cell(filLevel, colLevelRes)
      .string(rl.name)
      .style(styleName);

    let arrayRisk = risksHM?.filter(e => e.inherentRisk.riskLevel.id_risk_level === rl.id_risk_level);
    let arrayRiskRes = risksHM?.filter(e => e.residualRisk.riskLevel.id_risk_level === rl.id_risk_level);
    let sumLevel = 0;
    for (const x of arrayRisk) {
      const frequencyPer = x.percentage_inherent_risk_impact;
      const impactPer = x.percentage_inherent_risk_frequency;
      const producto = (frequencyPer && impactPer) ? frequencyPer * impactPer / 100 : 0;
      sumLevel += producto;
    }
    const promedio = arrayRisk?.length > 0 ? sumLevel / arrayRisk?.length : 0;
    let sumLevelRes = 0;
    for (const x of arrayRiskRes) {
      const frequencyPer = x.percentage_residual_risk_impact;
      const impactPer = x.percentage_residual_risk_frequency;
      const producto = (frequencyPer && impactPer) ? frequencyPer * impactPer / 100 : 0;
      sumLevelRes += producto;
    }
    const promedioRes = arrayRiskRes?.length > 0 ? sumLevelRes / arrayRiskRes?.length : 0;

    // cantidad
    ws.cell(filLevel, (colLevel + 1)).number(arrayRisk?.length)
      .style({
        alignment: { vertical: 'center', horizontal: 'center', },
      });

    ws.cell(filLevel, (colLevelRes + 1)).number(arrayRiskRes?.length)
      .style({
        alignment: { vertical: 'center', horizontal: 'center', },
      });

    // porcentaje
    ws.cell(filLevel, (colLevel + 2)).number((arrayRisk?.length / totalRisk))
      .style({
        alignment: { vertical: 'center', horizontal: 'center', },
        numberFormat: '0.00%'
      });

    ws.cell(filLevel, (colLevelRes + 2)).number((arrayRiskRes?.length / totalRisk))
      .style({
        alignment: { vertical: 'center', horizontal: 'center', },
        numberFormat: '0.00%'
      });
    ...
    // promedio
    ws.cell(filLevel, (colLevel + 3)).number((promedio / 100))
      .style({
        alignment: { vertical: 'center', horizontal: 'center', },
        numberFormat: '0.00%'
      });

    ws.cell(filLevel, (colLevelRes + 3)).number((promedioRes / 100))
      .style({
        alignment: { vertical: 'center', horizontal: 'center', },
        numberFormat: '0.00%'
      });
  ...
  let styleName = {
    alignment: { wrapText: true, vertical: 'center', horizontal: 'center', },
    fill: { type: 'pattern', patternType: 'solid', bgColor: '#000000', fgColor: '#000000' },
    border: {
      top: { style: 'thin', color: 'black' },
      bottom: { style: 'thin', color: 'black' },
      left: { style: 'thin', color: 'black' },
  ...
  colLevelRes = (colLevel + lenImpact + 4);
  ws.cell(filLevel, colLevel).string('Total').style(styleName);
  ws.cell(filLevel, colLevelRes).string('Total').style(styleName);

  ws.cell((filLevel - riskLevel.length - 1), (colLevel + 1)).string('N° Riesgos').style(styleName);
  ws.cell((filLevel - riskLevel.length - 1), (colLevelRes + 1)).string('N° Riesgos').style(styleName);
  // cantidad
  ws.cell(filLevel, (colLevel + 1)).number(totalRisk).style({ alignment: { vertical: 'center', horizontal: 'center', }, });
  ws.cell(filLevel, (colLevelRes + 1)).number(totalRisk).style({ alignment: { vertical: 'center', horizontal: 'center', }, });

  ws.cell((filLevel - riskLevel.length - 1), (colLevel + 2)).string('% Riesgos').style(styleName);
  ws.cell((filLevel - riskLevel.length - 1), (colLevelRes + 2)).string('% Riesgos').style(styleName);
  // porcentaje
  ws.cell(filLevel, (colLevel + 2)).number(1)
    .style({
      alignment: { vertical: 'center', horizontal: 'center', },
      numberFormat: '0.00%'
    });
  ws.cell(filLevel, (colLevelRes + 2)).number(1)
    .style({
      alignment: { vertical: 'center', horizontal: 'center', },
      numberFormat: '0.00%'
    });

  ws.cell((filLevel - riskLevel.length - 1), (colLevel + 3)).string('Riesgos Inherente (AVG)').style(styleName);
  ws.cell((filLevel - riskLevel.length - 1), (colLevelRes + 3)).string('Riesgos Inherente (AVG)').style(styleName);
  ...
  for (const r of risksHM) {
    ...
    ws.cell(numRow, 2).string(r.risk.name).style(stBody);
    ws.cell(numRow, 3).string(r.risk.reference).style(stBody);
    ...
    const per_fre_inh = r.percentage_inherent_risk_frequency; //frequencyRisk.find(e => e.id_frequency_risk === r.inherentRisk.frequencyRisk.id_frequency_risk).porcentaje;
    const per_imp_inh = r.percentage_inherent_risk_impact; //impactRisk.find(e => e.id_impact_risk === r.inherentRisk.impactRisk.id_impact_risk).porcentaje;
    ws.cell(numRow, 7).string(`${per_fre_inh?.toFixed(2)}%`).style(stBody).style({ alignment: { horizontal: 'center', vertical: 'center' } });
    ws.cell(numRow, 8).string(`${per_imp_inh?.toFixed(2)}%`).style(stBody).style({ alignment: { horizontal: 'center', vertical: 'center' } });

    const per_fre_res = r.percentage_residual_risk_frequency; //frequencyRisk.find(e => e.id_frequency_risk === r.residualRisk.frequencyRisk.id_frequency_risk).porcentaje;
    const per_imp_res = r.percentage_residual_risk_impact; //impactRisk.find(e => e.id_impact_risk === r.residualRisk.impactRisk.id_impact_risk).porcentaje;
    ws.cell(numRow, 9).string(`${per_fre_res?.toFixed(2)}%`).style(stBody).style({ alignment: { horizontal: 'center', vertical: 'center' } });
    ws.cell(numRow, 10).string(`${per_imp_res?.toFixed(2)}%`).style(stBody).style({ alignment: { horizontal: 'center', vertical: 'center' } });
    ...
    if (varImpact?.rating_type?.trim() === 'VARIABLES') {
      for (let a = 0; a < varImpact?.variables?.length; a++) {
        ...
        ws.cell(numRow, column).string(
          `${var_enc?.impactRisk?.description} ${((100 / lenImpact) * var_enc?.impactRisk?.weight)} %`
        ).style(stBody).style({ alignment: { horizontal: 'center', vertical: 'center' } });
        column++;
      }
    } else {
    ...
    if (varFrequency?.rating_type?.trim() === 'VARIABLES') {
      for (let c = 0; c < varFrequency?.variables?.length; c++) {
        ...
        ws.cell(numRow, column).string(
          `${var_enc?.frequencyRisk?.description} ${((100 / lenFrequency) * var_enc?.frequencyRisk?.weight)} %`
        ).style(stBody).style({ alignment: { horizontal: 'center', vertical: 'center' } });
        column++;
      }
    } else {
  ...
  /* hoja 05 */
  ws = wb.addWorksheet('RIESGO - CONTROL');
  //Head
  {
    let x = 1;
  ...
  for (const r of risksHM) {
    for (const c of r.control_risk) {
      ...
      ws.cell(numRow, x += 1).string(`${r.risk.name} - ${r.risk.reference}`).style(stBody);
