...
const getReport = async function (httpRequest) {
  ...
  for (let i = 0; i < risks[0].length; i++) {
    ...
    let stTmpIn = wb.createStyle({
      ...
      fill: {
        type: 'pattern',
        patternType: 'solid',
        bgColor: risks[0][i].risk_heatMap[0].inherentRisk.riskLevel.color,
        fgColor: risks[0][i].risk_heatMap[0].inherentRisk.riskLevel.color,
      }
    });
    let stTmpRe = wb.createStyle({
      font: {
        color: '#163508',
        size: 11,
      },
      ...
      fill: {
        type: 'pattern',
        patternType: 'solid',
        bgColor: risks[0][i].risk_heatMap[0].residualRisk.riskLevel.color,
        fgColor: risks[0][i].risk_heatMap[0].residualRisk.riskLevel.color,
      }
    });
    */
  ...
  //Head
  {
    ...
    ws.cell(1, 2).string("Riesgo").style(stHead);
    ws.cell(1, 3).string("Referencia riesgo").style(stHead);
    ws.cell(1, 4).string("Proceso").style(stHead);
    //ancho de la columna
    ws.column(1).setWidth(30);
    ws.column(2).setWidth(30);
  ...
  for (let i = 0; i < risks[0].length; i++) {
    ...
    for (let j = 0; j < risks[0][i]?.risk_heatMap[0]?.control_risk.length; j++) {
      ...
      // console.log('risks[0][' + i + ']?.risk_heatMap[0]?.control_risk[' + j + ']', risks[0][i]?.risk_heatMap[0]?.control_risk[j].controls.name)
      numRow++;
    }
  }
