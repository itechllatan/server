...
const saveControlsRiskFromControl = async function (httpRequest) {
  try {
    const language = httpRequest.headers?.Language;
    const controlsRiskRepository = new ControlsRiskRepository();
    const frequencyRiskRepository = new FrequencyRiskRepository();
    ...
    const saveInfo = await useCase.saveControlsRiskFromControl(httpRequest.body, httpRequest.user, language);
    return {
      statusCode: 201,
      body: saveInfo,
    };
