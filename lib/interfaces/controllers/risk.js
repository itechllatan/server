...
const getProcessByRiskId = async function (httpRequest){
  try{
    const { id } = httpRequest.params;
    if (!id) return { statusCode: 400, message: "Parametro Id es requerido" }
    ...
    const macroprocess = await useCase.getProcessByRiskId(id);
...
const deleteByRiskId = async function (httpRequest){
  try{
    const { id } = httpRequest.params;
    if(!id) return { statusCode: 400, message: "Parametro Id es requerido"}
    const ris = new PlanActionRiskRepository();
    const useCase = new Risk({
      planActionRiskRepository
    })
    const plans = await useCase.deleteByRiskId(id);
    return {
      statusCode: 200,
      body: plans
    }
