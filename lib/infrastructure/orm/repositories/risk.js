...
class RiskRepository extends CommonRepository {
  ...
  async getAllRisks(query) {
    ...
    const builder = await getConnection()
      ...
      .leftJoin('risk_heatMap.risk_cause_effect', 'risk_cause_effect')
      ...
      .leftJoin('risk_heatMap.riskFactorRisk', 'riskFactorRisk')
      .leftJoin('riskFactorRisk.risk_factor', 'risk_factor')
      .leftJoin('risk_heatMap.riskResponsible', 'riskResponsible')
    ...
    builder[1] = {
      page,
      pageSize,
      totalPages: (parseInt(builder[1]?.page / pageSize) || 0) + 1
    }
