...
class Risk {
    constructor({ riskRepository, controlRiskRepository, planActionRiskRepository, macroProcessRiskRepository }) {
    ...
    async getRisks() {
        const risks = await this.riskRepository.findAll({
            ...
            fields: ['id_risk', 'description', 'name','reference'],
        });
        return risks;
    }
    ...
    async getRiskById(id) {
        try {
            ...
            newRisk.calculateResidualRisk = risk.risk_heatMap[0]?.percentage_residual_risk_frequency && risk.risk_heatMap[0]?.percentage_residual_risk_impact ? 
            risk.risk_heatMap[0]?.percentage_residual_risk_frequency * risk.risk_heatMap[0]?.percentage_residual_risk_impact / 100 : 0;
            newRisk.calculateInherentRisk = risk.risk_heatMap[0]?.percentage_inherent_risk_frequency && risk.risk_heatMap[0]?.percentage_inherent_risk_impact ? 
            risk.risk_heatMap[0]?.percentage_inherent_risk_frequency * risk.risk_heatMap[0]?.percentage_inherent_risk_impact / 100 : 0;
    ...
    async getProcessByRiskId(id) {
        try{
            return await this.macroProcessRiskRepository.findAll({
                relations: ['macroProcessRisk', 'macroProcessRisk.risk_heat_map'],
                where: { 'risk_heat_map' : id}
            })
        }catch (e) {
            console.log(e)
    ...
    async getRisksText(text) {
        try {
            let builder = getConnection()
                ...
                .select([
                    'risk',//.id_risk',
                    'risk_heatMap',//.id_risk_heat_map',
                    'heatMap',//.id_heat_map',
                    'control_risk',//.id_controls_risk',
                    'controls',//.id_controls',

                    'inherentRisk',//.id_matrix',
                    'inherentRisk_riskLevel',//.id_risk_level',
                    'inherentRisk_frequencyRisk',//.id_frequency_risk',
                    'inherentRisk_impactRisk',//.id_impact_risk',

                    'residualRisk',//.id_matrix',
                    'residualRisk_riskLevel',//.id_risk_level',
                    'residualRisk_frequencyRisk',//.id_frequency_risk',
                    'residualRisk_impactRisk',//.id_impact_risk',
                ])
                .from('risk', 'risk')
                ...
                .andWhere(`(risk.name like :text 
                    or risk.description like :text)`)
