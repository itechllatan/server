...
class Risk {
    ...
    async getRisks(query) {
        return await this.riskRepository.getAllRisks(query);
    }
    ...
    async insertRisk(newRisk, user) {
        try {
            ...
            await getConnection().transaction(async entityManager => {
                ...
                risk = savedRisk;
                newRisk.business_continuity = newRisk?.risk_heatMap?.[0]?.business_continuity;
                newRisk.residualRisk = newRisk.inherentRisk;
        ...
        try {
            return await this.controlRiskRepository.findAll({
                relations: ['controls', 'risk_heat_map', 'risk_heat_map.risk'
                    , 'controls.solidityGeneral'
                    , 'controls.solidityDesign'
                    , 'controls.solidityExecution'
                ],
        ...
        try {
            let builder = getConnection()
                ...
                .andWhere(`(upper(risk.name) like :text or 
                             upper(risk.description) like :text) and 
                             "risk".deleted_at `)
