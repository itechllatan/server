...
class ControlsRiskUC {
  ...
  */
  async saveControlsRisk(info, user, language) {
    ...
    await getConnection().transaction(async entityManager => {
      try {
        ...
        console.log('e', e)
        throw new SaveError('No se pudo guardar', 'link', 'not save');
      }
    })
  ...
  async saveControlsRiskFromControl(info, user, language) {
    ...
    await getConnection().transaction(async entityManager => {
      try {
        this.control = new ControlsRisk({
          validators: {},
          user: user.id_user,
          ...info,
        });

        saveInfo = await entityManager.getRepository(ControlsRiskSchema).save(this.control);
        const controlsRisk = await this.controlsRiskRepository.findAll({
          relations: ['controls', 'risk_heat_map', 'risk_heat_map.heatMap'],
          where: { risk_heat_map: saveInfo?.risk_heat_map?.id_risk_heat_map, deleted_at: null }
        })
