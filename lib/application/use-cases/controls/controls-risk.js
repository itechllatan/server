...
class ControlsRiskUC {
  ...
  */
   ...
   */
  // async saveControlsRisk(info, user, language) {
  //   console.log("🚀🥵 ~ file: controls-risk.js:279 ~ ControlsRiskUC ~ saveControlsRisk ~ info:", info)
  //   const mess = language && language === 'en' ? message_en : message;
  //   let saveInfo;
  //   await getConnection().transaction(async entityManager => {
  //     try {
  ...
  //       );
  //       saveInfo = await entityManager.getRepository(ControlsRiskSchema).save(this.controls);
  //       console.log("🚀🥵 ~ file: controls-risk.js:293 ~ ControlsRiskUC ~ awaitgetConnection ~ saveInfo:", saveInfo)
  //       if (saveInfo?.length) {
  //         let x = 0;
  //         // const controlsPromises = saveInfo?.map(async (control) => {
  //         //   console.log("🚀🥵 ~ file: controls-risk.js:303 ~ ControlsRiskUC ~ controlsPromises ~ control:", control?.risk_heat_map?.id_risk_heat_map, control?.controls?.id_controls, control)
  //         //   const controlRisk = await this.controlsRiskRepository.findOne({
  //         //     relations: ['controls', 'risk_heat_map', 'risk_heat_map.heatMap'],
  //         //     where: { risk_heat_map: control?.risk_heat_map?.id_risk_heat_map, controls: control?.controls?.id_controls, deleted_at: null }
  //         //   })
  //         //   console.log("🚀🥵 ~ file: controls-risk.js:301 ~ ControlsRiskUC ~ controlsPromises ~ controlRisk:", controlRisk)
  //         //   x++;
  //         //   return controlRisk[0];
  //         // });
  //         // const results = await Promise.all(controlsPromises);
  //         // const controlsSearch = [...results];
  //         // console.log("🚀🥵 ~ file: controls-risk.js:304 ~ ControlsRiskUC ~ awaitgetConnection ~ controlsSearch:", controlsSearch)

  //         const controlsSearch = [];
  //         for (const control of saveInfo) {
  //           console.log("🚀🥵 ~ file: controls-risk.js:303 ~ ControlsRiskUC ~ controlsPromises ~ control:", control?.risk_heat_map?.id_risk_heat_map, control?.controls?.id_controls, control)
  //           const controlRisk = await this.controlsRiskRepository.findAll({
  //             relations: ['controls', 'risk_heat_map', 'risk_heat_map.heatMap'],
  //             where: { risk_heat_map: control?.risk_heat_map?.id_risk_heat_map, controls: control?.controls?.id_controls, deleted_at: null }
  //           })
  //           x++;
  //           console.log("🚀🥵 ~ file: controls-risk.js:301 ~ ControlsRiskUC ~ controlRisk:", controlRisk);
  //           controlsSearch.push(controlRisk);
  //         };
      ...
      try {
        if (saveInfo?.length) {
          ...
          for (const control of saveInfo) {
            ...
            x++;
            console.log("🚀🥵 ~ file: controls-risk.js:301 ~ ControlsRiskUC ~ controlRisk:", controlRisk);
            controlsSearch.push(controlRisk.constructor === Array ? controlRisk[0] : controlRisk);
          };

          console.log("🚀🥵 ~ file: controls-risk.js:455 ~ ControlsRiskUC ~ awaitgetConnection ~ controlsSearch:", controlsSearch)
          let id_heat_map;
          let id_risk_heat_map;
          let quantityControls = 0;
          let impactControl = 0;
        ...
        console.log('error saveControlsRisk: ', e)
        throw new SaveError(e, 'link', 'not save');
      }
    })
