...
class SubprocessUC {
  ...
  async getSubprocessByProcess(id, query) {
    return await this.subprocessRepository.getAllSubprocessByProcess(id, query);
    // return await this.subprocessRepository.findAll({
    //   relations: ['user', 'typeProcess', 'categoryProcess', 'process'],
  ...
  async deleteSubprocess(id, user) {
    try {
      let updDelete;
      await getConnection().transaction(async entityManager => {
        updDelete =
