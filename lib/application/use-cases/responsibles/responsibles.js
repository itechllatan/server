...
class ResponsiblesUC {
    ...
    async getResponsibles(query) {
        return await this.responsiblesRepository.getAllResponsibles(query);
    ...
    async deleteResponsibles(id, user) {
        try {
            let updDelete;
            await getConnection().transaction(async entityManager => {
                updDelete =
                    await this.responsiblesRepository.update(
                        { id_responsibles: id },
