    ...
    async getReportRisk() {
        try {
            ...
            const someQuery = await entityManager.query(
                `SELECT rhm.id_risk_heat_map "id_risk_heat_map"
                , hm.id_heat_map "id_heat_map", hm.name "name_heat_map"
                , r.id_risk "id_risk", r.name "name_risk", r.reference "reference_risk", nvl(r.description, ' ') "description_risk"
                FROM MR_RISK_HEAT_MAP rhm
                JOIN MR_HEAT_MAP hm ON hm.id_heat_map = rhm.id_heat_map
                JOIN MR_RISKS r ON r.id_risk = rhm.id_risk
                ORDER BY hm.id_heat_map, r.id_risk`);
            return someQuery
        } catch (e) {
            console.log(e);
        }
    ...
    async getReport() {
        try {
            ...
            const someQuery = await entityManager.query(
                `SELECT rhm.id_risk_heat_map "id_risk_heat_map"
                , hm.id_heat_map "id_heat_map", hm.name "name_heat_map"
                , nvl(to_char(rhm.percentage_inherent_risk_frequency, 'FM9G999' ) || '%', ' ') "percentage_inherent_risk_frequency"
                , nvl(to_char(rhm.percentage_inherent_risk_impact, 'FM9G999') || '%', ' ') "percentage_inherent_risk_impact"
                , nvl(to_char(rhm.percentage_residual_risk_frequency, 'FM9G999') || '%', ' ') "percentage_residual_risk_frequency"
                , nvl(to_char(rhm.percentage_residual_risk_impact, 'FM9G999') || '%', ' ') "percentage_residual_risk_impact"
                , rl.name "inherent_risk", rl2.name "residual_risk"
                , r.id_risk "id_risk", r.reference "reference_risk", r.name "name_risk"
                ...
                JOIN MR_HEAT_MAP hm ON hm.id_heat_map = rhm.id_heat_map
                JOIN MR_RISKS r ON r.id_risk = rhm.id_risk
                JOIN MR_MATRIX m on m.id_matrix= rhm.id_matrix_inherent
                JOIN MR_MATRIX m2 on m2.id_matrix = rhm.id_matrix_residual
                JOIN MR_RISK_LEVEL rl ON rl.id_risk_level = m.id_risk_level
                JOIN MR_RISK_LEVEL rl2 ON rl2.id_risk_level = m2.id_risk_level
