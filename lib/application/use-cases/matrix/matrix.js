...
class Matrix {
    ...
    async updateMatrix (body, user){
        ...
        /*console.log(matrixToUpdate.map(column => column.map(field => field.id_matrix)))
        console.log(matrixToUpdate.map(column => column.map(field => field.id_matrix)).toString());*/
        const frequencyIds = frequencyRisk.map(element => element.id_frequency_risk).filter(element => element !== undefined);
        console.log("FREQUENCY IDS ",frequencyIds);
        const allFrequencyRisk = await this.frequencyRepository.findAll({
            where: {heatMap: heatMap.id }
        })
        console.log("ALL FREQUENCY RISK ",allFrequencyRisk)
        console.log(impactRisk.map(element => element.id_impact_risk).filter(element => element !== undefined).toString());
        console.log(impactRisk);
        console.log(riskLevel);
        console.log(user);
        //Eliminar ids no encontrados en objeto recibido (Matriz, Frecuencias, Impactos y Niveles de Riesgo)

        //Insertar y actualizar elementos recibidos en el objeto (Matriz, Frecuencias, Impactos y Niveles de Riesgo)
    }
}
