...
class Matrix {
    constructor({ matrixRepository, frequencyRepository, impactRepository, heatMapRepository, riskLevelRepository, riskHeatMapRepository }) {
    ...
    async getMatrixByHeatMap(id) {
        ...
        const riskHeatMapQuantity = await this.riskHeatMapRepository.findAll({
            where: { heatMap: id, deleted_at: null }
        })
        let idFrequency = 0;
        let tempArray = [];
        const matrixArray = [];
        ...
        impactRisk = impactRisk.sort((a, b) => a.id_impact_risk - b.id_impact_risk);
        matrixArray.push(tempArray);
        return [
            matrixArray,
            riskLevel,
    ...
    async getRiskLevelByHeatMap(id) {
        return await this.riskLevelRepository.findAll({
            where: { heatMap: id },
        });
    }
}
